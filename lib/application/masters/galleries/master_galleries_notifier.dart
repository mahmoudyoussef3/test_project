...
class MasterGalleriesNotifier extends StateNotifier<MasterGalleriesState> {
  final MastersFacade _mastersRepository;
  final SettingsFacade _settingsRepository;

  MasterGalleriesNotifier(this._mastersRepository, this._settingsRepository)
      : super(const MasterGalleriesState());
  ...
  Future<void> setGalleries(BuildContext context) async {
    ...
    final response = await _mastersRepository.setGalleries(
    ...
    response.when(
      ...
      failure: (failure, status) {
        state = state.copyWith(isUpdating: false);
        AppHelpers.errorSnackBar(context, text: failure);
        debugPrint('==> set Master galleries failure $failure');
      },
    );
  }

  Future<void> fetchMasterGalleries(
      {VoidCallback? checkYourNetwork, VoidCallback? onSuccess}) async {
    final connected = await AppConnectivity.connectivity();
    if (connected) {
      state = state.copyWith(isLoading: true);
      final response = await _mastersRepository.getGalleries();
      response.when(
        success: (data) async {
          state =
              state.copyWith(isLoading: false, listOfUrls: data, images: []);
        },
        failure: (failure, status) {
          state = state.copyWith(isLoading: false);
          debugPrint('==> get Master galleries failure: $failure');
        },
      );
    } else {
